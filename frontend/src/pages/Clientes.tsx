...
import { ClientMessageMenu } from '../components/clients/ClientMessageMenu';
...
const Clientes = () => {
  ...
  const [viewingPreferences, setViewingPreferences] = useState<Customer | null>(null);
  ...
  const [availableServices, setAvailableServices] = useState<SimpleServicePackage[]>([]);

  const fetchServicesList = async () => {
    try {
      const token = localStorage.getItem('token');
      // Using fetch directly to avoid modifying shared api.ts for now
  ...
  return (
    <div className="min-h-full pb-24">
          ...
          <div className={`flex p-1 rounded-full border shadow-sm w-fit ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            {[
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === tab.key
      ...
      >
        <p className={`text-sm mt-1 mb-4 text-center sm:text-left ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Gerencie contatos, contratos e histórico.</p>
      </GradientHeader>

      <div className={`${pageGutters} max-w-full md:max-w-6xl mx-auto space-y-6 mt-6`}>
        ...
        {activeTab === 'list' ? (
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {statusFilterOptions.map((option) => (
                  <button
                    ...
                    className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold border transition-all ${statusFilter === option.value
                ...
                clientes.map((cliente, index) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 pt-0.5">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 mb-2">
                          <span className={`w-1.5 h-1.5 rounded-full ${cliente.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-amber-500'
                            }`} />
                    ...
                    <div className={`grid grid-cols-2 gap-2 mt-3 pt-3 border-t ${isDark ? 'border-slate-800' : 'border-slate-50'}`}>
                      <ClientMessageMenu customer={cliente} isDark={isDark} />
                      <button
                        onClick={() => setViewingPreferences(cliente)}
                      ...
                      >
                        <Heart size={12} className={cliente.clientPreferences ? 'fill-rose-500 text-rose-500' : ''} />
                        Preferências
                      </button>
                      <button
                        onClick={() => handleViewHistory(cliente)}
                        className={`col-span-2 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border text-xs font-bold transition-colors ${isDark ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                      >
                        <FileText size={12} />
                        Histórico
                      </button>
          ...
          <div className="animate-fade-in space-y-6">
            {wizardStatus && (
              <div className={`p-4 rounded-2xl border flex items-center gap-3 ${wizardStatus.type === 'success'
                            ...
                            <div className={`absolute left-0 right-0 top-full mt-1 rounded-xl shadow-lg border z-50 max-h-48 overflow-y-auto ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                              {addressSuggestions.map((prediction) => (
                                <button
                                  ...
                                  className={`w-full text-left px-4 py-3 text-sm transition-colors border-b last:border-0 ${isDark
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          >
                            ...
                            {/* Always allow keeping the current value if it's not in the list (legacy support) */}
                            {formData.serviceType && !availableServices.some(s => s.name === formData.serviceType) && (
                              <option value={formData.serviceType}>{formData.serviceType} (Legacy)</option>
                            )}
                          </select>
                        </div>
                        <div>
        ...
        {viewingPreferences && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className={`w-full max-w-md rounded-3xl p-6 shadow-2xl scale-100 animate-scale-in transition-all ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className={`p-3 rounded-full ${isDark ? 'bg-rose-900/30 text-rose-400' : 'bg-rose-50 text-rose-500'}`}>
                    <Heart size={20} className="fill-current" />
                  </div>
                  <div>
                    <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Preferências</h3>
                    <p className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                      {viewingPreferences.name}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setViewingPreferences(null)}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {viewingPreferences.clientPreferences?.focusAreas?.length ? (
                    <div className="flex flex-wrap gap-2">
                      {viewingPreferences.clientPreferences.focusAreas.map((area) => (
                        <span
                          key={area}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${isDark
                    ...
                    <p className={`text-sm italic ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>
                      Nenhuma área selecionada pelo cliente.
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <p className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                    Fragrância Preferida
                  </p>
                  <div className={`flex items-center gap-3 p-3 rounded-2xl border ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                    <span className="text-xl">🌸</span>
                    <span className={`text-sm font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                      {viewingPreferences.clientPreferences?.fragrance || 'Não especificado'}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className={`p-4 rounded-2xl border min-h-[80px] ${isDark ? 'bg-slate-800/30 border-slate-700 text-slate-300' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                    <p className="text-sm leading-relaxed">
                      {viewingPreferences.clientPreferences?.notes || 'Nenhuma observação adicional.'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                <button
                  onClick={() => setViewingPreferences(null)}
                  className={`px-6 py-2.5 rounded-xl text-sm font-bold ${isDark
              ...
              {appointments.map((app) => (
                <div key={app.id} className="flex gap-4 relative">
                  <div className="flex flex-col items-center">
                    <div className={`w-3 h-3 rounded-full mt-1.5 ${app.status === 'CONCLUIDO' ? 'bg-emerald-400' : 'bg-slate-300'
                      }`} />
