...
const Profile = () => {
  ...
  const handlePortalSubmit = async (e?: React.FormEvent) => {
    ...
    if (!portalForm.customerId) {
      setPortalError('Selecione um cliente.');
      return;
    }

    try {
      setPortalSaving(true);
      const response = await teamApi.generatePortalLink(portalForm.customerId);
      setPortalMessage({ email: response.link, password: '' }); // Store link in email field
    } catch (err: any) {
      setPortalError(err?.response?.data?.error || 'Erro ao gerar link.');
    } finally {
  ...
  return (
    <>
      <div className={`min-h-full ${isDarkTheme ? 'bg-[#0f172a]' : 'bg-[#f6f7fb]'}`}>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className={`text-xs font-medium ${isDarkTheme ? 'text-slate-400' : 'text-slate-500'}`}>Gerar Magic Link para cliente</p>
                </div>
              </div>
        ...
        {/* Magic Link Portal Modal */}
        {portalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">Portal do Cliente</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Gere um link de acesso exclusivo</p>
                </div>
                <button
                  onClick={() => {
                    setPortalOpen(false);
                    setPortalForm({ customerId: '', name: '', email: '', password: '' });
                    setPortalMessage(null);
                    setPortalError('');
                  }}
              ...
              <div className="p-6 space-y-6">
                ...
                {portalMessage && (
                  <div className="space-y-3 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
                    <div className="flex items-start gap-3">
                      <div className="p-2 bg-emerald-100 dark:bg-emerald-900/40 rounded-lg">
                        <CheckCircle2 size={20} className="text-emerald-600 dark:text-emerald-400" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-bold text-emerald-900 dark:text-emerald-100 mb-1">Link Gerado com Sucesso!</p>
                        <p className="text-xs text-emerald-700 dark:text-emerald-300">
                          Compartilhe este link com {portalForm.name}
                        </p>
                      </div>
                    </div>

                    <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-emerald-200 dark:border-emerald-800/50">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(portalMessage.email);
                          alert('Link copiado!');
                        }}
                        className="flex-1 px-4 py-2 bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-800/50 text-emerald-700 dark:text-emerald-300 rounded-lg text-sm font-bold hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-colors"
                      >
                        Copiar Link
                      </button>
                      <button
                        onClick={() => {
                          const text = encodeURIComponent(
                            `Olá ${portalForm.name}! Aqui está o link para acessar seu portal: ${portalMessage.email}`
                          );
                          window.open(`https://wa.me/${portalForm.name.replace(/\D/g, '')}?text=${text}`, '_blank');
                        }}
                        className="flex-1 px-4 py-2 bg-emerald-600 dark:bg-emerald-700 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors flex items-center justify-center gap-2"
                      >
                ...
                <button
                  ...
                  disabled={!portalForm.customerId || portalSaving}
                  className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white font-bold rounded-xl transition-colors disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {portalSaving ? (
                    <>
                ...
                <p className="text-xs text-center text-slate-500 dark:text-slate-400">
                  O cliente poderá acessar o portal através deste link único e permanente
                </p>
              </div>
            </div>
          </div>
